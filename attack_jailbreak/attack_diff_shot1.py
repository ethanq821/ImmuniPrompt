...
def main():
    ...
    filedata = load_jsonl('/data/jiani/prompt_new/dataset/jailbreak/26512_re-number.jsonl')
    ...
    for model_names in model_list:
        print(model_names)
        prompts_with_resp = '/data/jiani/prompt_new/llamaguard_judge/attack_jailbreak/'+model_names+'/goal_hijack.jsonl'
        for data in filedata:
            print("processing ",data['id'], model_names, gpu_id)
            prompt = data['prompt']
            response,load_time, run_time  = test_model(goal_hijack+prompt, model_names, system_prompt)
