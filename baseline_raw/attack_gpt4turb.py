...
def judgment(response):
    ...
    sys_prompt = """
        ...
        Your output must be either "True" or "False".
...
def attack_gpt(datas,test_model,output_file):

    with open(output_file, "a", encoding='utf-8') as file:
        for data in datas:
            if data.get('jailbreak'):
                text = data['prompt']
                response = test_gpt(text,test_model)
...
def main():
    ...
    success_file = '/data/jiani/prompt_new/test_origin/result/gpt4turb/1_t.jsonl'
    output_file = '/data/jiani/prompt_new/test_origin/result/gpt4turb/1.jsonl'
    ...
    print('---------------------attack start-------------------------')
    start = time.time()
    #attack_gpt(datas,test_model,output_file)
    end = time.time()
    print(f'attack time in {model_name}: {end - start}')
    print('---------------------attack over-------------------------')
