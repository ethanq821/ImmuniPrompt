...
def test_model(prompt, target_model, sys_prompt="NULL"):
    ...
    if(target_model == 'gpt4o'):
        ...
        response, loadtime, runtime = local_mistral3(sys_prompt,prompt)
...
def main():
    ...
    for model_names in model_list:
        print(model_names)
        prompts_with_resp = '/data/jiani/prompt_new/llamaguard_judge/attack_jailbreak/'+model_names+'/2000_simple_shot1.jsonl'
        for data in filedata:
            print("processing ",data['id'], model_names, gpu_id)
            prompt = data['prompt']
            response,load_time, run_time  = test_model(prompt, model_names, system_prompt)
