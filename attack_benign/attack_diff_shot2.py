...
def main():
    ...
    for model_names in model_list:
        print(model_names)
        prompts_with_resp = '/data/jiani/prompt_new/llamaguard_judge/attack_benign/'+model_names+'/2000_raw.jsonl'
        for data in filedata:
            # if(data['id']<4418):
            #     continue
            print("processing ",data['id'], model_names, gpu_id)
            prompt = data['prompt']
            response, load_time, run_time = test_model(prompt, model_names)
