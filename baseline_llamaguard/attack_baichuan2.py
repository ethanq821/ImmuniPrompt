...
print('------------------------------------Loading start----------------------------------------')
safes = []
with open('/data/jiani/prompt_new/test_llamaguard/llama_guard2_query_output14_safe.jsonl', 'r') as f:
    for line in f:
...
start = time.time()
with open('/data/jiani/prompt_new/test_llamaguard/results/attack_baichuan2.jsonl', "a", encoding='utf-8') as file:
    for safe_prompt in safes:
            response = test_model(safe_prompt, 'baichuan2')
            judgment_result = judgment(response)
            print(judgment_result)
            output_record = {
                'prompt': safe_prompt,
                'attack': judgment_result,
                'response': response,
...
print('-----------------------------------Saving start----------------------------------------') 
with open('/data/jiani/prompt_new/test_llamaguard/Attack_Times.jsonl', "a", encoding='utf-8') as file:
    output_record = {
        'time': end-start,
        'model': 'baichuan2'
    }
